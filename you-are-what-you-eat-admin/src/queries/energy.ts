...
class EnergyApi {
    ...
    public getCurrentAvailable: () => Promise<EnergyPanelData[]> = async () => {
        ...
        const today_power = today.filter((e) => e.type === '电表:KwH')[0].consumption;
        const week_power  = week.filter((e) => e.type === '电表:KwH')[0].consumption;
        // console.log('today', today_power);
        const today_water = today.filter((e) => e.type === '水表:Ton')[0].consumption;
    ...
    public getOverallUsage: () => Promise<OverallUsageType> = async () => {
        ...
        const weekTotalArr = weekOriConsumptionArray.map((dailyData) => arrSum(dailyData, (e) => e.consumption));
        const weekTotal  = arrSum(weekTotalArr, (d) => d);
        const weekGasTotal = arrSum(weekOriConsumptionArray.map((dailyData) => dailyData[0]), (e) => e.consumption);
        const weekPwrTotal = arrSum(weekOriConsumptionArray.map((dailyData) => dailyData[1]), (e) => e.consumption);
        const weekWtrTotal = arrSum(weekOriConsumptionArray.map((dailyData) => dailyData[2]), (e) => e.consumption);
        const usage: OverallUsageType = {
            view_by_type: [
                {
                    type: '总量',
    ...
    public getYearlyData: () => Promise<YearlyEnergyDataSortedByType[]> = async () => {
        ...
        const powerConsumptionList = monthOriConsumptionArray.map((e) => e[1].consumption);
        const powerSum = arrSum(powerConsumptionList, (e) => e);
        const gasConsumptionList   = monthOriConsumptionArray.map((e) => e[0].consumption);
