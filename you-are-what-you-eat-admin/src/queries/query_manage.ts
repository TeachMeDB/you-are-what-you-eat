...
class QueryManageApi {
  public getManageList: () => Promise<ManageInfo[]> = async () => {
    try {
      ...
      const r = await (
        await fetch(
          `http://106.14.212.200:8000/app/api/Asset/GetAssetRecord?token=${token}`
        )
      ).text();
  ...
  public addManage: (params: any) => Promise<string> = async (params) => {
    try {
      return await (
        await fetch(
          'http://106.14.212.200:8000/app/api/manages/Asset/PostAddAssetRecord',
          {
            method: 'POST',
  ...
  public updateManage: (params: any) => Promise<string> = async (params) => {
    try {
      return await (
        await fetch(
          'http://106.14.212.200:8000/app/api/Asset/PostUpdateAssetRecord',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...params, token })
