...
class QueryIngredientApi {
  public getIngredientList: (ingrName: string) => Promise<IngredientInfo[]> =
    async (ingrName = '') => {
      try {
        const r = await (
          await fetch(
            `http://106.14.212.200:8000/app/api/Ingredient/GetIngredient?ingrName=${ingrName}&token=${token}`
          )
        ).text();
  ...
  public addIngredient: (params: any) => Promise<string> = async (params) => {
    try {
      return await (
        await fetch(
          'http://106.14.212.200:8000/app/api/Ingredient/PostAddIngredient',
          {
            method: 'POST',
  ...
  public updateIngredient: (params: any) => Promise<string> = async (
    ...
    try {
      return await (
        await fetch(
          'http://106.14.212.200:8000/app/api/Ingredient/PostUpdateIngredient',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...params, token })
  ...
  public deleteIngredient: (id: any) => Promise<string> = async (id) => {
    try {
      return await (
        await fetch(
          `http://106.14.212.200:8000/app/api/Ingredient/DeleteIngredient?id=${id}&token=${token}`,
          { method: 'DELETE' }
        )
      ).text();
    } catch (err) {
